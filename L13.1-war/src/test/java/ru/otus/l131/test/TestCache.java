...
class TestCache {
	...
	void testMaxCapacity2() {
		...
		AddressDataSet theAddress = (AddressDataSet)cache.get(new DataSetKey(address.getClass(), address.getId())).getValue();
		assertEquals(address, theAddress);

		// Max capacity = 2, third inserted entry should be in cache
		PhoneDataSet thePhone = (PhoneDataSet)cache.get(new DataSetKey(phone.getClass(), phone.getId())).getValue();
