...
public class SocketMessageServer implements SocketMessageServerMBean {
    ...
    private final ExecutorService messageRouter;
    private final ConcurrentMap<Address, MessageWorker> workers;
    
    private volatile boolean running = false;
     
    public SocketMessageServer() {
        messageRouter = Executors.newSingleThreadExecutor();
    ...
    public void start() throws Exception {
        ...
        messageRouter.execute(this::routeMessages);
        ...
        while (running) {
       		for (MessageWorker worker : workers.values()) {
       			...
       			if (message != null) {
       				...
       				try {
        				route(message);
        				logger.info("Message routed to : " + message.getTo());
    ...
    private void sendShutdownMessage() {
       	for (MessageWorker worker : workers.values()) {
       		try {
       			worker.put(new Message(null, null, new ShutdownMessage()));
    ...
    private void route(Message message) throws InterruptedException {
